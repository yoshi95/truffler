...
Truffler.prototype._run = function(url, options, done) {
	...
	async.series({
		...
		randomizePort: guardAgainstTimeout(function(next) {
			if (options.page || optoins.browser) {
				return next();
			}
		...
		createBrowser: guardAgainstTimeout(function(next) {
			if (options.page || optoins.browser) {
				options.log.info('skipping create browser because browser/page passed in...');
				return next();
			}
		...
		createPage: guardAgainstTimeout(function(next) {
			if (options.page || optoins.browser) {
				options.log.info('skipping create page because browser/page passed in...');
				return next();
			}
		...
		applyPageHeaders: guardAgainstTimeout(function(next) {
			if (options.page || optoins.browser) {
				options.log.info('skipping custom headers because browser/page passed in...');
				return next();
			}
		...
		applyPageSettings: guardAgainstTimeout(function(next) {
			if (options.page || optoins.browser) {
				options.log.info('skipping custom page settings because browser/page passed in...');
				return next();
			}
		...
		openUrl: guardAgainstTimeout(function(next) {
			if (options.page || optoins.browser) {
				options.log.info('skipping url location browser/page passed in...');
				return next();
			}
